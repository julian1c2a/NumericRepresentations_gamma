...
namespace detail {
    template <uint64_t n> struct floorsqrt_ct_helper {
      ...
      static constexpr uint64_t x0 = (x0_base * x0_base < n) ? (x0_base * 2) : x0_base;
...
template <typename T> constexpr T floorsqrt(T n) noexcept {
  ...
  if (x0 * x0 < un) x0 <<= 1; // Ajuste si subestimamos

  // Newton-Raphson
  UnsignedT x1 = (x0 + un / x0) / 2;
  while (x1 < x0) { x0 = x1; x1 = (x0 + un / x0) / 2; }
