#ifndef NUMREPR_APPEND_INT128_OPS_HPP
#define NUMREPR_APPEND_INT128_OPS_HPP
...
namespace NumRepr {
    ...
    struct uint128_t {
        ...
        constexpr uint128_t(uint64_t l, uint64_t h) noexcept : low(l), high(h) {}
        ...
        constexpr const uint128_t& operator=(T v) noexcept {
            low = static_cast<uint64_t>(v);
            // Manejo correcto de números negativos asignados a unsigned (ej: a = -1)
            high = (is_signed_v<T> && v < 0) ? ~uint64_t{0} : 0;
            return *this;
        }
        ...
        constexpr strong_ordering operator<=>(const uint128_t& rhs) const noexcept {
            if (auto cmp = high <=> rhs.high; cmp != 0) return cmp;
            return low <=> rhs.low;
        }

        // --- Operadores Bit a Bit ---
        constexpr uint128_t operator|(const uint128_t& rhs) const noexcept { return {low | rhs.low, high | rhs.high}; }
        constexpr uint128_t operator&(const uint128_t& rhs) const noexcept { return {low & rhs.low, high & rhs.high}; }
        constexpr uint128_t operator^(const uint128_t& rhs) const noexcept { return {low ^ rhs.low, high ^ rhs.high}; }
        constexpr uint128_t operator~() const noexcept { return {~low, ~high}; }
        ...
        constexpr const uint128_t& operator<<=(int shift) noexcept { *this = *this << shift; return *this; }
        constexpr const uint128_t& operator>>=(int shift) noexcept { *this = *this >> shift; return *this; }
        ...
        constexpr const uint128_t& operator+=(const uint128_t& rhs) noexcept { *this = *this + rhs; return *this; }
        ...
        static constexpr std::pair<uint128_t, uint128_t> divmod(uint128_t n, uint128_t d) noexcept {
            ...
            for(int i = 127; i >= 0; --i) {
                r <<= 1;
                uint64_t bit = (i >= 64) ? ((n.high >> (i - 64)) & 1) : ((n.low >> i) & 1);
                if (bit) r.low |= 1;
                if (r >= d) {
                    r -= d;
                    if (i >= 64) q.high |= (1ULL << (i - 64)); else q.low |= (1ULL << i);
                }
            }
            return {q, r};
        }

        constexpr uint128_t operator/(const uint128_t& rhs) const noexcept { return divmod(*this, rhs).first; }
        constexpr uint128_t operator%(const uint128_t& rhs) const noexcept { return divmod(*this, rhs).second; }
        constexpr const uint128_t& operator/=(const uint128_t& rhs) noexcept { *this = *this / rhs; return *this; }
        constexpr const uint128_t& operator%=(const uint128_t& rhs) noexcept { *this = *this % rhs; return *this; }
    };
    ...
    struct sint128_t {
        ...
        constexpr strong_ordering operator<=>(const sint128_t& rhs) const noexcept {
            if (auto cmp = high <=> rhs.high; cmp != 0) return cmp;
            return low <=> rhs.low;
        }

        constexpr uint128_t as_unsigned() const { return {low, static_cast<uint64_t>(high)}; }
        static constexpr sint128_t from_unsigned(uint128_t u) { return {u.low, static_cast<std::int64_t>(u.high)}; }
        ...
        constexpr const sint128_t& operator++() noexcept {
            uint128_t u = as_unsigned(); ++u; *this = from_unsigned(u); return *this;
        }
        ...
        constexpr const sint128_t& operator+=(const sint128_t& rhs) noexcept { *this = *this + rhs; return *this; }
        ...
        constexpr sint128_t operator/(const sint128_t& rhs) const noexcept {
            ...
            uint128_t u_lhs = neg_lhs ? (-*this).as_unsigned() : as_unsigned();
            uint128_t u_rhs = neg_rhs ? (-rhs).as_unsigned() : rhs.as_unsigned();
        ...
        constexpr sint128_t operator%(const sint128_t& rhs) const noexcept {
            bool neg_lhs = (high < 0);
            uint128_t u_lhs = neg_lhs ? (-*this).as_unsigned() : as_unsigned();
            uint128_t u_rhs = (rhs.high < 0) ? (-rhs).as_unsigned() : rhs.as_unsigned();
            uint128_t res = u_lhs % u_rhs;
    ...
    template<>
    inline constexpr uint64_t mulmod<uint64_t>(uint64_t a, uint64_t b, uint64_t m) noexcept {
        if (m == 0) return 0;
        if (a == 0 || b == 0) return 0;
        if (m == 1) return 0;
        if (a == 1) return (b % m);
    ...
    namespace internal {
        ...
        consteval T mulmod_ct_impl() {
            if constexpr (b == 0) return result;
                ...
                constexpr T next_res = ((b & 1) ? ((result >= mod - a) ? (result + a - mod) : (result + a)) : result);
        ...
        consteval uint64_t mulmod_ct_impl<uint64_t, a, b, mod, result>() {
            if constexpr (b == 0) return result;
            else {
                constexpr uint64_t next_a = (a >= mod - a) ? (a * 2 - mod) : (a * 2);
                constexpr uint64_t next_res = ((b & 1) ? ((result >= mod - a) ? (result + a - mod) : (result + a)) : result);
                constexpr uint64_t next_b = (b >> 1);
                return mulmod_ct_impl<uint64_t, next_a, next_b, mod, next_res>();
            }
        }
    }
    ...
    consteval uint64_t mulmod_ct<uint64_t, A, B, Mod>() {
        ...
        if constexpr (A == 1) return B % Mod;
        if constexpr (B == 1) return A % Mod;
        return internal::mulmod_ct_impl<uint64_t, A % Mod, B, Mod, 0>();
    }

} // namespace NumRepr

#endif // NUMREPR_APPEND_INT128_OPS_HPP
