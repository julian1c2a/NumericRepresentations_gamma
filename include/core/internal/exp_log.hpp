...
enum class math_error_ec : uint8_t {
  ok, notsupported, baddomain, overflow, underflow, unknown
};
...
namespace LUT {
    ...
    consteval size_t max_exponent_for_base_ct() noexcept {
        ...
        else return 1; // Fallback seguro para bases grandes no tabuladas
    }
...
constexpr inline uint64_t int_pow(uint64_t base, uint32_t exponent) noexcept {
    ...
    if (exponent > LUT::max_exponent_for_base(base)) return 0;
    ...
    while (exponent > 0) {
        if (exponent & 1) {
            if (result > numeric_limits<uint64_t>::max() / cur) return 0;
            result *= cur;
        }
        exponent >>= 1;
        if (exponent > 0) {
            if (cur > numeric_limits<uint64_t>::max() / cur) return 0;
            cur *= cur;
        }
    }
    return result;
...
template <uint64_t base, int64_t n> constexpr int64_t int_log_ct() noexcept {
  ...
  else return 1 + int_log_ct<base, n / base>();
}

constexpr int64_t int_log(uint64_t base, int64_t n) noexcept {
  if (base <= 1) return 0; // Error
  if (n <= 0) return -1;
...
constexpr inline bool pow_leq_limit(uint64_t b, uint32_t exp, uint64_t limit) noexcept {
    ...
    while (exp > 0) {
        if (exp & 1) {
            if (result > limit / cur) return false;
            result *= cur;
        }
        exp >>= 1;
        if (exp > 0) {
            if (cur > numeric_limits<uint64_t>::max() / cur) return false;
            cur *= cur;
        }
    }
    return result <= limit;
...
constexpr inline size_t count_digits_base(uint64_t n, uint64_t base) noexcept {
  ...
  while (pow_leq_limit(base, hi, n)) { lo = hi; hi *= 2u; if (hi==0) break; }

  while (lo + 1u < hi) {
    uint32_t mid = lo + (hi - lo) / 2u;
    if (pow_leq_limit(base, mid, n)) lo = mid; else hi = mid;
  }
