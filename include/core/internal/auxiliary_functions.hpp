...
namespace AuxFunc {
  ...
  template <std::uint64_t n>
  constexpr std::uint64_t int_log2_sf_ct() noexcept {
  ...
  constexpr std::uint64_t int_pow2_sf_ct() noexcept {
  ...
  constexpr std::expected<std::uint64_t,math_error_ec> int_log2_sf(std::uint64_t n) noexcept {
    if (n == 0) {
      return std::unexpected(math_error_ec::baddomain);
    }
	if (n == 1) {
      return (0ull);
    }
  ...
  constexpr std::expected<std::uint64_t,math_error_ec> int_pow2(std::size_t exponent) noexcept {
    if (exponent < 64)
		return (1ull << exponent);
	else
		return std::unexpected(math_error_ec::overflow);
  } // END FUNCTION int_pow2 runtime
